# Product Requirements Document (PRD) & Architecture Blueprint: Playtrafi

**Document Version**: 1.1.0  
**Status**: Active / Approved  
**Author**: marcuz-apl / Playtrafi Project  
...
**Playtrafi** (formerly *Patchtroy*) is a standalone open-source Python library, CLI, and microservice designed to convert any dynamic web page into clean, LLM-ready Markdown and structured data with resilient execution across modern web perimeters.

By coupling **Patchright** (the C++-patched undetected Playwright engine) with **Trafilatura** (the gold standard in algorithmic boilerplate removal and Markdown extraction), Playtrafi delivers superior stealth and content cleanliness with zero machine learning dependencies.
...
### 2.2 The Playtrafi Solution
Playtrafi occupies the unoccupied **"Lightweight Stealth Sweet Spot"**:
...
- **FR-01: Asynchronous & Synchronous Python API**
  - Provide `AsyncPlaytrafi` with async context management (`async with`).
  - Provide synchronous `Playtrafi` convenience methods (`Playtrafi.crawl(url)`).
  - Retain backwards-compatible `AsyncPatchtroy` and `Patchtroy` aliases.
...
- **FR-08: Command-Line Interface (CLI)**
  - Provide a standalone terminal executable `playtrafi <url>` (with alias `patchtroy`) supporting `-o/--output`, `--format [markdown|json|html|csv]`, `--wait-for`, and `--headful`.
...
- [x] Core drivers and Trafilatura Markdown extraction.
...
### Phase 3 (v0.3.0)
- [x] Lightweight Docker container (`marcuszou/playtrafi:latest`).
...
- [x] Full test suite (38 tests passing, 100% ruff clean, wheel package verified).

### Phase 5 (v0.6.0 — Rebranding & Transition)
- [x] Rebrand project to **Playtrafi** (`playtrafi` on PyPI).
- [x] Preserve backwards compatibility aliases for `Patchtroy`, `AsyncPatchtroy`, and `PatchtroyConfig`.
- [x] Transitional package support for `patchtroy` on PyPI.
